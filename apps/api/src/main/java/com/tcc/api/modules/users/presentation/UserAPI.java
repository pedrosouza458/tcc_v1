...
public interface UserAPI {
        ...
        @ApiResponses({
                        ...
                        @ApiResponse(responseCode = "409", description = "Conflito: E-mail ou CPF já cadstrados no sistema")
        })
        ResponseEntity<Map<String, String>> createUser(@Valid @RequestBody UserRequestDTO request);

        @Operation(summary = "Listar alunos", description = "Retorna uma lista paginada dos alunos da instiuição.")
        @ApiResponses({
        ...
        public ResponseEntity<Page<UserResponseDTO>> listUsersByInstitution(
                        @RequestParam UUID instutitionId,
        ...
        public ResponseEntity<Map<String, UserResponseDTO>> getUserById(
                        @Parameter(name = "id", description = "ID do usuário", example = "a9db0a7f-cb21-4f2c-8536-227f81670604", required = true) UUID id);
}
